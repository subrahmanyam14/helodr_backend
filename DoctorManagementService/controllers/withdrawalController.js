...
exports.recordHospitalTransfer = async (req, res) => {
  ...
  try {
    ...
    // Create admin transfer transaction
    const adminTransferTransaction = await Transaction.createTransaction({
      user: req.user._id,
    ...
    withdrawal.adminTransferTransaction = adminTransferTransaction._id;
    await withdrawal.save({ session });
    ...
    res.json({
      success: true,
      message: "Hospital transfer recorded successfully",
      data: withdrawal
    });
